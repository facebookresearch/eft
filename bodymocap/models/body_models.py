...
class SMPLX(SMPLH):
    ...
    def forward(self, betas=None, global_orient=None, body_pose=None,
        ...
        if pose2rot == True:
            ...
            full_pose = torch.cat([global_orient, body_pose], dim=1)                #(N, 22, 3, 3)
            
            full_others = torch.cat([jaw_pose, leye_pose, reye_pose,
                                left_hand_pose,
                                right_hand_pose], dim=1)
            # full_pose += self.pose_mean
            ...
            full_pose_others = batch_rodrigues(full_others.view(-1,3),dtype=self.dtype).view([batch_size, -1, 3, 3 ])       #(N, 33, 3, 3)

            full_pose = torch.cat([full_pose, full_pose_others], dim=1)         #(4,55, 3, 3)
