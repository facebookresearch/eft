...
from eft.utils.imutils import uncrop
from eft.utils.pose_utils import reconstruction_error


from eft.utils.imutils import convert_smpl_to_bbox, convert_bbox_to_oriIm, conv_bboxinfo_center2topleft#, deNormalizeBatchImg
# from utils.part_utils import PartRenderer

# Define command-line arguments
parser = argparse.ArgumentParser()
parser.add_argument('--checkpoint', default=None, help='Path to network checkpoint')
parser.add_argument('--dataset', default='h36m-p1', choices=['coco2014-val-3d-amt','h36m-p1', 'h36m-p2','ochuman-test', 'lsp','lspet-test','3dpw', '3dpw-vibe', '3dpw-crop', '3dpw-headcrop', 'mpi-inf-3dhp-test' ,'all'], help='Choose evaluation dataset')
...
parser.add_argument('--force', action="store_true", help='If true, ignore existing results and always rerun')
...
def eval_main(params):
    ...
    if os.path.exists(evalLogFileName):      #Bug recompute all
        print(f"Found already processed log: {evalLogFileName}")
    ...
    evalLogAll['total_step_count'] = checkpoint['total_step_count']
    evalLogAll['batch_size'] = checkpoint['batch_size']
    evalLogAll['epoch'] = checkpoint['epoch']
    ...
    if args.dataset =='all':        #Process all quantitative evaluations
        ...
        datasetList =['coco2014-val-3d-amt','3dpw-crop', '3dpw-headcrop' ,'h36m-p2', 'h36m-p1', '3dpw', '3dpw-vibe', 'mpi-inf-3dhp-test','lspet-test', 'ochuman-test']
    else:
    ...
    if os.path.exists("/private/home/hjoo/codes/fairmocap/benchmarks_eval"):
        ...
        strcmd = f"cp -v {evalLogFileName} /private/home/hjoo/codes/fairmocap/benchmarks_eval/{targetFile}"
        os.system(strcmd)
