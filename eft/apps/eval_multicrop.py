...
# parser.add_argument('--dataset', default='h36m-p1', choices=['h36m-p1', 'h36m-p2', 'lsp', '3dpw', '3dpw-crop', '3dpw-headcrop', 'mpi-inf-3dhp' ,'all'], help='Choose evaluation dataset')
...
def run_evaluation(model, dataset_name, dataset, result_file,
    ...
    if dataset_name == 'h36m-p1' or dataset_name == 'h36m-p2' or '3dpw' in dataset_name or dataset_name == 'mpi-inf-3dhp' :
        eval_pose = True
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        if False:
            #show cropped image
            # curImgVis = img
            curImgVis = de_normalize_img(images[0]).cpu().numpy()
...
def eval_main(params):
    ...
    if args.dataset =='all':        #Process all quantitative evaluations
        ...
        datasetList =['3dpw-crop', '3dpw-headcrop' ,'h36m-p2', 'h36m-p1', '3dpw', 'mpi-inf-3dhp']
    if args.dataset =='all':        #Process all quantitative evaluations
        datasetList =['3dpw_vibe_test_multilevel_0', '3dpw_vibe_test_multilevel_1' ,'3dpw_vibe_test_multilevel_2', '3dpw_vibe_test_multilevel_4', '3dpw_vibe_test_multilevel_7', '3dpw-vibe']
    else:
        datasetList =[args.dataset]
    datasetList = [n for n in datasetList if n not in evalLogAll ]      #Ignore already processed one
    for n in evalLogAll:
        if evalLogAll[n]==-1:
            datasetList.append(n)
...
if __name__ == '__main__':
    ...
    if len(sys.argv)>1:
        ...
        params +=['--dataset','3dpw_test_multilevel_4']        
        # params +=['--num_workers',0]
