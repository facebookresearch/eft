...
def rotmat3x3_to_angleaxis(init_pred_rotmat):
    ...
    pred_aa = tgm.rotation_matrix_to_angle_axis(pred_rotmat_hom).contiguous().view(1, -1)  #[1,72]
...
def run_evaluation(model, dataset_name, dataset, result_file,
    ...
    if g_smpl_neutral is None:
        ...
        smplModelPath = '/home/hjoo/codes/frankmocap/extra_data/smpl/SMPLX_NEUTRAL.pkl'
        g_smplx_neutral = SMPLX(smplModelPath,
                batch_size=1,
                num_betas = 10,
                use_pca = False,
    ...
    JOINT_REGRESSOR_TRAIN_EXTRA_SMPLX = '/home/hjoo/codes/frankmocap/extra_data/body_module/J_regressor_extra_smplx.npy'
    J_regressor_smplx = np.load(JOINT_REGRESSOR_TRAIN_EXTRA_SMPLX)           #(9, 10475)

    import pickle
    with open('/home/hjoo/codes_test/expose/expose_data/data/SMPLX_to_J14.pkl','rb') as f:
        J_regressor_smplx_14 = pickle.load(f, encoding='latin1')            #(14, 10475)
    J_regressor_smplx_14 = J_regressor_smplx_14.astype(np.float32)
    J_regressor_smplx_14_batch = torch.from_numpy(J_regressor_smplx_14[None,:]).cuda()
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        if bLoadFromFile:
            ...
            pklDir= '/home/hjoo/codes/frankmocap/3dpw'
            ...
            for subjectid, name in zip(batch['subjectId'], batch['imgname']):
                ...
                # pklfilename ='downtown_arguing_00_image_00006_pid0.pkl'
                ...
                if os.path.exists(pklfilepath) == False:
                    ...
                    with open(pklfilepath,'rb') as f:
                        ...
                        pred_rotmat_list.append(data['pred_pose_rotmat'][None,:])
                        pred_betas_list.append(data['pred_shape'][None,:])
                        pred_camera_list.append(data['pred_camera'][None,:])
        ...
        pred_output = g_smplx_neutral(betas=pred_betas, body_pose=pred_rotmat[:,1:], global_orient=pred_rotmat[:,[0]], pose2rot=False)
        # # pred_output = g_smplx_neutral(betas=pred_betas, body_pose=pred_aa[:,3:], global_orient=pred_aa[:,:3], pose2rot=True)
        ...
        if eval_pose:
            ...
            if False:       #Original XMPL
                # Get 14 predicted joints from the mesh
                pred_keypoints_3d = torch.matmul(J_regressor_batch, pred_vertices)          #(1, 17, 3)
                if save_results:
                    pred_joints[step * batch_size:step * batch_size + curr_batch_size, :, :]  = pred_keypoints_3d.cpu().numpy()
                
                pred_keypoints_3d = pred_keypoints_3d[:, joint_mapper_h36m, :]          #(1,14,3)
                pred_pelvis = pred_keypoints_3d[:, [0],:].clone()
                pred_keypoints_3d = pred_keypoints_3d - pred_pelvis 

            else:       #SMPL-X
                pred_vertices = pred_vertices.detach()
                pred_keypoints_3d = torch.matmul(J_regressor_smplx_14_batch, pred_vertices)          #(1, 17, 3)
                pred_pelvis = (pred_keypoints_3d[:, [2],:].clone() +pred_keypoints_3d[:, [3],:].clone()) *0.5
                pred_keypoints_3d = pred_keypoints_3d - pred_pelvis 
