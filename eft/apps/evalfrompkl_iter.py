...
def run_evaluation(model, dataset_name, dataset, result_file,
    ...
    # outputPath='reconErrorData_05-24_3dpw_test_with1336_iterUpto20.pkl'
    outputPath='reconErrorData_05-25_3dpw_test_with1336_iterUpto50.pkl'
    if os.path.exists(outputPath):
        import pickle
        
        with open(outputPath,'rb') as f:
            reconError = pickle.load(f)       #Bug fixed
            f.close()
    else:
        for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
            ...
            if bLoadFromFile:
                ...
                pklDir= '/run/media/hjoo/disk/data/cvpr2020_eft_researchoutput/0_SPIN/0_exemplarOutput/05-25_3dpw_test_with1336_iterUpto50'
                pklDir= '/private/home/hjoo/spinOut/06-02_3dpw_test_with8653_3dpwbest_iterUpto50'

                pklDir= '/private/home/hjoo/spinOut/11-01_3dpw_test__vibe_with8377_cocoAl_h36_inf_3dpw_iterUpto50'
                ...
                for name in batch['imgname']:
                    ...
                    if os.path.exists(pklfilepath) == False:
                        ...
                        with open(pklfilepath,'rb') as f:
                            ...
                            reconError.append(data['reconErrorInfo'])
        ...
        if True:
            import pickle
            # outputPath='reconErrorData.pkl'
            with open(outputPath,'wb') as f:
                pickle.dump(reconError,f)       #Bug fixed
                f.close()
    reconErrorPerIter=[]
    for it in range(50):
        print(it)
    ...
    for it in range(50):
        print(it)
        keyPtErrorPerIter.append([d[it][1] for d in reconError])


    # viewer2D.Plot(reconError)
    reconErrorPerIter = np.array(reconErrorPerIter)
    keyPtErrorPerIter = np.array(keyPtErrorPerIter)

    
    for it in range(50):
        print("{}: reconError:{}, keyptError{}".format(it, np.mean(reconErrorPerIter[it,:]),np.mean(keyPtErrorPerIter[it,:])     ))


    #Find the best
    bestIter = np.argmin(reconErrorPerIter,axis=0)
    print("Best: reconError:{}".format(np.mean(np.min(reconErrorPerIter,axis=0))))

    #Thresholding by keypointError
    thresh_keyptErr_2d = 2e-4# self.option.eft_thresh_keyptErr_2d_testtime #2*1e-4
    reconErrorByThreshold =[]
    threshIter =[]
    for fIdx  in range(keyPtErrorPerIter.shape[1]):
        for iterIdx  in range(20):
            if keyPtErrorPerIter[iterIdx][fIdx] < thresh_keyptErr_2d:
                break
        
        if iterIdx>19:
            iterIdx =19

        threshIter.append(iterIdx)
        reconErrorByThreshold.append(reconErrorPerIter[iterIdx][fIdx])

    
    print("ByThreshold: reconError:{} with thresh{}".format(np.mean(reconErrorByThreshold),thresh_keyptErr_2d ))
    # viewer2D.Plot(threshIter)
    # viewer2D.Plot(bestIter)
