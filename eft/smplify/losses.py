...
from eft.utils.geometry import perspective_projection, weakProjection_gpu
...
def body_fitting_loss_weakperspective(body_pose, betas, model_joints, camera_t,
                      joints_2d, joints_conf, pose_prior, sigma=100, pose_prior_weight=4.78,
                      shape_prior_weight=5, angle_prior_weight=15.2,
                      output='sum'):
    """
    Loss function for body fitting
    ...
    #TODO: Disable pose and angle prior
    # pose_prior_weight = 0
    # angle_prior_loss = 0
    ...
    # projected_joints = perspective_projection(model_joints, rotation, camera_t,
                                            #   focal_length, camera_center)
    projected_joints = weakProjection_gpu(model_joints, camera_t[:,0], camera_t[:,1:] )           #N, 49, 2

    # Weighted robust reprojection error
    scaleFactor = 112       # -112 and 112 space originally used in SIMPLify, to balance with other terms. 
    reprojection_error = gmof(projected_joints *scaleFactor - joints_2d*scaleFactor, sigma)

    joints_conf[:,:25] *=0      #Disable Openpose
    ...
    if output == 'sum':
        return total_loss.sum(), reprojection_loss
    elif output == 'reprojection':
        return reprojection_loss


def body_prior_loss(body_pose, betas, 
                         pose_prior,
    ...
    total_prior_loss = pose_prior_loss + angle_prior_loss + shape_prior_loss

    return total_prior_loss.sum()
...
def camera_fitting_loss_weakperspective(model_joints, camera_t, camera_t_est, joints_2d, joints_conf, depth_loss_weight=100):
    """
    Loss function for camera optimization.
    """
    ...
    #Han: Disable openpose for SMPlify. Bad assocation happens (openpose and GT are from different people)
    # reprojection_loss = (is_valid * reprojection_error_op + (1-is_valid) * reprojection_error_gt).sum(dim=(1,2))      
